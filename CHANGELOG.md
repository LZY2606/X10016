...
* ✨ Add `container` block rule and `attr_span` inline rule, with a shared `{...}` attribute block parser in `markdown_it.helpers.parse_attrs`. The block rule parses `:::name {#id .class key=value}` fences (3 or more colons) into a `container_open`/`container_close` token pair (tag `div`, `markup` set to the opening colons, unstripped `info`, name in `meta["name"]`, attributes in `attrs` with a leading `container`/`container-<name>` class), recursively parsing the content as block-level markdown; the inline rule parses `[text]{#id .class key=value}` into a `span_open`/`span_close` pair (tag `span`) with recursively parsed inline content. Both rules are registered on the default rulers, so they can be toggled with `md.enable(...)`/`md.disable(...)`; they are enabled in the `default` preset and disabled in the `commonmark`, `zero` and `gfm-like` presets, whose rendering is unchanged unless the rules are explicitly enabled.
